#!/bin/bash
### BEGIN INIT INFO 
# Provides: 		 xms_daemon_maintain_nas
# Required-Start:   
# Required-Stop:  
...
# ou sudo update-rc.d xms_daemon_maintain_nas.sh defaults 5 (5 est le 5eme à etre exécuté)
# et sudo update-rc.d -f xms_daemon_maintain_nas.sh remove
...
case "$1" in
   ...
   'setup')
        ...
        sudo ln -s /etc/init.d/$scriptName /etc/rc5.d/"K03"$scriptName 2>/dev/null
